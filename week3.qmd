...
```
![result of hazeDN (3 Bands)](image/hazeDN.png)
...
m1_NDVI %>%
  plot(.)
```
![result of m1_NDVI](image/m1.png)
...
![area of vegetation](image/veg.png)
...
Use band 5 and band 6 on the occasion of Landsat8:
...
```
![result of m1_NDMI](image/NDMI.png)
```
moi <- m1_NDMI %>%
...
plot(glcm.red)
```
![result of glcm.red](image/glcm_red.png)
```
plot(glcm.nir)
```
![result of glcm_nir](image/glcm_nir.png)
...
summary(pca$model)
```
![result of pca (m1_raster, glcm.red)](image/pca.png)
...
plot(pca$map)
```
![plot of PCA each component (m1_raster, glcm.red)](image/pcas.png)
...
summary(pca$model)
```
![result of pca (glcm.red, glcm.nir)](image/pca2.png)
